...
int main(int argc, char *argv[])
{
	...
	while (1)
	{
		...
		*client_fd = accept(server_fd, (struct sockaddr *)&client_addr, &addr_len);
		...
		ParametrosThread parametrosThread;
		parametrosThread.cantidadFrases=&cantidadFrases;
		parametrosThread.frases=frases;
		parametrosThread.cliente_id=client_fd;

		if (pthread_create(&tid, NULL, handle_client, &parametrosThread) != 0)
		{
